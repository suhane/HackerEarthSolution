...
public class DuplicateInArray {
    ...
    public static int repeatedNumber(final List<Integer> a) {
        ...
        for(int num:a){
            range= (int) Math.ceil((double)num/r);
            if(freq.containsKey(range)) {
                freq.put(range, freq.get(range) + 1);
                if(freq.get(range)>r)
                    break;
            }
            else
